...
void Dictionary::insert(TrieNode *curr, unsigned int code, unsigned char ch, bool isDescompress){
...
TrieNode* Dictionary::find(TrieNode *curr, unsigned char ch){
...
TrieNode* Dictionary::findCode(unsigned int code){
  if(code == 0)
    return this->getRoot();
  else if(this->codesAdd.size() < code)
    return nullptr;
...
std::string Dictionary::getStringToNode(TrieNode *curr){
  ...
  while(curr != this->getRoot()){
    buffer = char(curr->character) + buffer;
    curr = curr->parent;
  }
